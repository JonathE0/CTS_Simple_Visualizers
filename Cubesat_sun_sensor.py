...
const colors=['#ff948b','#bd5557','#89dab1','#439876','#86b9ff','#506dc4'];
...
function drawScene(){
 ...
 const origin=[43,h-85];ctx.font='10px system-ui';ctx.fillStyle='#b7a7a8';ctx.fillText('WORLD',20,h-120);
...
function drawHistory(){const [w,h]=fit(chart,hc);hc.clearRect(0,0,w,h);const left=30,top=9,bottom=h-20,width=w-left-8,height=bottom-top;hc.font='10px system-ui';hc.lineWidth=1;
 for(const value of [0,.5,1]){const y=bottom-value*height;hc.strokeStyle='#332426';hc.beginPath();hc.moveTo(left,y);hc.lineTo(w,y);hc.stroke();hc.fillStyle='#b7a7a8';hc.fillText(String(value),0,y+3);}
 hc.fillStyle='#b7a7a8';hc.fillText('-15 s',left,h-3);hc.fillText('now',w-25,h-3);
